...
class MiniGrid(embodied.Env):
  ...
  def __init__(self, task, obs_key="image", act_key="action"):
    ...
    self._full_render_shape = (
        self._env.width * self._tile_size,
        self._env.height * self._tile_size,
        3,
    )
  ...
  def full_obs(self):
    ...
    full_grid[self._env.agent_pos[0]][self._env.agent_pos[1]] = np.array(
        [OBJECT_TO_IDX["agent"], COLOR_TO_IDX["red"], self._env.agent_dir]
    )

    return full_grid

  def full_obs_rendered(self):
    return self._env.render(mode="rgb_array", highlight=True, tile_size=self._tile_size)
