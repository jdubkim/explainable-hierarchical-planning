...
def main(argv=None):
  ...
  try:
    ...
    env = embodied.envs.load_env(config.task,
                                 mode='train',
                                 logdir=logdir,
                                 **config.env)
    ...
    if config.run == 'train':
      ...
      eval_env = embodied.envs.load_env(config.task,
                                        mode='eval',
                                        logdir=logdir,
                                        **config.env)
      ...
      if config.eval_dir:
        ...
        eval_replay = make_replay('eval_episodes', config.replay_size // 10)
      replay = make_replay('episodes', config.replay_size)
      train_with_viz.train_with_viz(agent, env, replay, eval_replay, logger,
                                    args)
    elif config.run == 'learning':
      assert config.replay.sync
      env.close()
      replay = make_replay('episodes', config.replay_size)
      eval_replay = (make_replay(config.eval_dir, config.replay_size //
                                 10) if config.eval_dir else replay)
