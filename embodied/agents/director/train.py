...
def main(argv=None):
  ...
  if config.run == "acting":
    ...
    logger = embodied.Logger(
        step, output_list, multiplier=config.env.repeat * parsed.actors
    )
  else:
    output_list = [
        embodied.logger.TerminalOutput(config.filter),
  ...
  try:
    ...
    env = embodied.envs.load_env(config.task, mode="train", logdir=logdir, **config.env)
    ...
    if config.run == "train":
      ...
      if config.eval_dir:
        ...
        eval_replay = make_replay("eval_episodes", config.replay_size // 10)
        final_eval_replay = make_replay("final_eval_episodes", config.replay_size // 10)

      replay = make_replay("episodes", config.replay_size)
      train_with_viz.train_with_viz(
          agent, env, replay, eval_replay, logger, args, final_eval_replay
      )
