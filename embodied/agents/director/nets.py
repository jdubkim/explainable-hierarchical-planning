...
class MultiEncoder(tfutils.Module):
  ...
  def __init__(
    ...
    self.mlp_shapes = {
        k: v for k, v in shapes.items() if re.match(mlp_keys, k) and len(v) in (0, 1)
    }
...
class MultiDecoder(tfutils.Module):
  ...
  def __init__(
    ...
    if self.cnn_shapes:
      ...
      if cnn == "simple":
        self._cnn = ImageDecoderSimple(merged, cnn_depth, cnn_kernels, strides, **kw)
      else:
        raise NotImplementedError(cnn)
    if self.mlp_shapes:
...
class ImageEncoderSimple(tfutils.Module):
  ...
  def __init__(self, depth, kernels, strides, **kw):
    ...
    self._strides = (
        strides if isinstance(strides, (list, tuple)) else [strides] * len(kernels)
    )
    self._kw = kw
...
class ImageDecoderSimple(tfutils.Module):
  ...
  def __call__(self, features):
    ...
    for i, (kernel, stride) in enumerate(zip(self._kernels[:-1], self._strides[:-1])):
      x = self.get(f"conv{i}", ConvT, depth, kernel, stride, **self._kw)(x)
      depth //= 2
    x = self.get("out", ConvT, self._shape[-1], self._kernels[-1], self._strides[-1])(x)
    x = tf.math.sigmoid(x)
