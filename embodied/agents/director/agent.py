...
class Agent(tfagent.TFAgent):
    ...
    def __init__(self, obs_space, act_space, step, config, renderer=None):
        self.config = config
        ...
        if config.expl_behavior == 'None':
            ...
            self.expl_behavior = getattr(behaviors, config.expl_behavior)(
                self.wm, self.act_space, self.config, self.renderer)
    ...
    def policy(self, obs, state=None, mode='train'):
        ...
        outs = {
            **outs, 'action':
            tfutils.action_noise(outs['action'], noise, self.act_space)
        }
    ...
    def train(self, data, state=None):
        ...
        if 'key' in data:
            ...
            outs.update(key=data['key'],
                        priority=criteria[self.config.priority])
        ...
        if self.config.data_loader == 'tfdata':
            ...
            return tf.data.Dataset.range(self.config.batch_size).interleave(
                lambda _: tf.data.Dataset.from_generator(
                    generator, dtypes, shapes), ).batch(
                        self.config.batch_size).prefetch(tf.data.AUTOTUNE)
        elif self.config.data_loader == 'embodied':
            return embodied.Prefetch(sources=[generator] *
...
class WorldModel(tfutils.Module):
    ...
    def loss(self, data, state=None, training=False):
        ...
        if not self.config.tf.debug_nans:
            if 'reward' in dists:
                stats = tfutils.balance_stats(dists['reward'], data['reward'],
                                              0.1)
    ...
    def imagine(self, policy, start, horizon):
        ...
        traj = tfutils.scan(step, tf.range(horizon), start,
...
class ImagActorCritic(tfutils.Module):
        ...
        self.actor = nets.MLP(
            ...
            **self.config.actor,
            dist=(config.actor_dist_disc
                  if act_space.discrete else config.actor_dist_cont))
        self.grad = (config.actor_grad_disc
                     if act_space.discrete else config.actor_grad_cont)
        self.advnorm = tfutils.Normalize(**self.config.advnorm)
        self.retnorms = {
            k: tfutils.Normalize(**self.config.retnorm)
            for k in self.critics
        }
        self.scorenorms = {
            k: tfutils.Normalize(**self.config.scorenorm)
            for k in self.critics
        }
        if self.config.actent_perdim:
            shape = act_space.shape[:-1] if act_space.discrete else act_space.shape
            self.actent = tfutils.AutoAdapt(shape,
                                            **self.config.actent,
                                            inverse=True)
        else:
    ...
    def train(self, imagine, start, context):
        def policy(s):
            return self.actor(tf.nest.map_structure(tf.stop_gradient,
                                                    s)).sample()
    ...
    def target(self, traj, reward, impl):
        ...
        if impl == 'gae':
            ...
            for t in reversed(range(len(disc))):
                advs.append(deltas[t] +
                            disc[t] * self.config.return_lambda * advs[-1])
            adv = tf.stack(list(reversed(advs))[:-1])
            return adv + value[:-1], value[:-1]
        elif impl == 'gve':
            vals = [value[-1]]
            interm = reward + disc * value[1:] * \
                (1 - self.config.return_lambda)
            for t in reversed(range(len(disc))):
                vals.append(interm[t] +
                            disc[t] * self.config.return_lambda * vals[-1])
