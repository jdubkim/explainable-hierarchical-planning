...
class WorldModel(tfutils.Module):
    ...
    def imagine_carry(self, policy, start, horizon, carry):
        ...
        def transp(x): return {k: [x[t][k]
                                   for t in range(len(x))] for k in x[0]}
...
class ImagActorCritic(tfutils.Module):
    ...
    def __init__(self, critics, scales, act_space, config):
        ...
        if self.config.actent_perdim:
            shape = act_space.shape[:-
                                    1] if act_space.discrete else act_space.shape
    ...
    def train(self, imagine, start, context):
        def policy(s): return self.actor(tf.nest.map_structure(
    ...
    def target(self, traj, reward, impl):
        ...
        if impl == 'gae':
            ...
            for t in reversed(range(len(disc))):
                advs.append(deltas[t] + disc[t] *
                            self.config.return_lambda * advs[-1])
            ...
            for t in reversed(range(len(disc))):
                vals.append(interm[t] + disc[t] *
                            self.config.return_lambda * vals[-1])
