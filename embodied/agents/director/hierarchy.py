...
class Hierarchy(tfutils.Module):
  ...
  def policy(self, latent, carry, imag=False):
    ...
    # Log encoded states.
    if self.config.log_enc:
      feat = self.feat(latent)
      outs["log_enc"] = self.enc({"goal": feat, "context": feat}).mode()
    outs["log_skill"] = skill
  ...
  def train_jointly(self, imagine, start):
    ...
    with tf.GradientTape(persistent=True) as tape:
      ...
      traj = self.wm.imagine_carry(
          policy, start, self.config.imag_horizon, self.initial(len(start["is_first"]))
      )
