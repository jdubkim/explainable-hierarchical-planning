...
class Hierarchy(tfutils.Module):
    ...
    def initial(self, batch_size):
        return {
            ...
            'skill': tf.zeros((batch_size, ) + self.config.skill_shape,
                              tf.float32),
    ...
    def policy(self, latent, carry, imag=False):
        ...
        switch = lambda x, y: (tf.einsum(
            'i,i...->i...', 1 - update.astype(x.dtype), x) + tf.einsum(
                'i,i...->i...', update.astype(x.dtype), y))
        skill = sg(
            switch(carry['skill'],
                   self.manager.actor(sg(latent)).sample()))
        new_goal = self.dec({
            'skill': skill,
            'context': self.feat(latent)
        }).mode()
    ...
    def train_jointly(self, imagine, start):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            traj = self.wm.imagine_carry(policy, start,
    ...
    def train_jointly_old(self, imagine, start):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            traj['goal'] = tf.repeat(goal[None], 1 + self.config.imag_horizon,
                                     0)
            traj['skill'] = tf.repeat(skill[None],
                                      1 + self.config.imag_horizon, 0)
            traj['reward_extr'] = self.extr_reward(traj)
            traj['reward_expl'] = self.expl_reward(traj)
            traj['reward_goal'] = self.goal_reward(traj)
            traj['delta'] = traj['goal'] - self.feat(traj).astype(tf.float32)
    ...
    def goal_reward(self, traj):
        ...
        if self.config.goal_reward == 'dot':
            ...
            return tf.einsum('...i,...i->...', tf.nn.l2_normalize(goal, -1),
                             feat)[1:]
            ...
            return tf.einsum('...ij,...ij->...', probs / norm,
                             skill / norm)[1:]
    ...
    def abstract_traj(self, traj):
        ...
        for key, value in list(traj.items()):
            if 'reward' in key:
                ...
                traj[key] = tf.concat([reshape(value[:-1])[:, 0], value[-1:]],
                                      0)
        traj['weight'] = tf.math.cumprod(
            self.config.discount * traj['cont']) / self.config.discount
        return traj
    ...
    def report_worker(self, data, impl):
        ...
        states, _ = self.wm.rssm.observe(
            self.wm.encoder(data)[:6], data['action'][:6],
            data['is_first'][:6])
        ...
        traj = self.wm.imagine(worker, start,
                               self.config.worker_report_horizon)
        # Decoder into images.
